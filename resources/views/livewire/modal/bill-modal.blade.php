    ...
    @else
        @if ($editing)
            <x-input label="Nama Pelanggan" wire:model="customer_name" type="text" disabled
                placeholder="Automatis di buat" />

            <x-input label="Invoice" wire:model="invoice" type="text" disabled
                placeholder="Automatis di buat" />
            ...
            <x-input label="Jatuh Tempo" wire:model.blur="due_date" type="date" margin="mb-0"
                placeholder="Pilih jatuh tempo" :error="$errors->first('due_date')"
        ...
        @else {{-- CREATING --}}
            ...
            <x-input label="Invoice" wire:model="invoice" type="text" readonly placeholder="Automatis di buat"
                :error="$errors->first('invoice')" />
            ...
            <x-input label="Periode" wire:model.change="period" type="month" :required="$required"
                placeholder="Pilih periode" :error="$errors->first('period')" :disabled="!$customerInfo" :readonly="$customerInfo && !$customerInfo->last_bill" :margin="$customerInfo && !$customerInfo->last_bill ? 'mb-0' : 'mb-3'"
                min="{{ now()->format('Y-m') }}" max="{{ now()->addMonth()->format('Y-m') }}" />
            ...
            <x-input label="Jatuh Tempo" wire:model.blur="due_date" type="date" margin="mb-0"
                placeholder="Pilih jatuh tempo" :error="$errors->first('due_date')" :disabled="!$customerInfo || !$period" min="{{ now()->format('Y-m-d') }}"
                max="{{ now()->addMonth(2)->format('Y-m-d') }}" />
